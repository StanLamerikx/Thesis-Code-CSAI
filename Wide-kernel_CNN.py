...
X_train_ptbdb, X_temp_ptbdb, y_train_ptbdb, y_temp_ptbdb = train_test_split(
    X_ptbdb, y_ptbdb, test_size=0.3, random_state=42, stratify=y_ptbdb
)
X_val_ptbdb, X_test_ptbdb, y_val_ptbdb, y_test_ptbdb = train_test_split(
    X_temp_ptbdb, y_temp_ptbdb, test_size=0.5, random_state=42, stratify=y_temp_ptbdb
)
...
# Define WideKernelCNN
class WideKernelCNN(nn.Module):
    def __init__(self, input_dim, num_classes):
        super(WideKernelCNN, self).__init__()
        self.layer1 = nn.Sequential(
            nn.Conv1d(1, 64, kernel_size=25, stride=1, padding=12),  # Wide kernel size
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2)
        )
        self.layer2 = nn.Sequential(
            nn.Conv1d(64, 128, kernel_size=25, stride=1, padding=12),  # Wide kernel size
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2)
        )
        self.layer3 = nn.Sequential(
            nn.Conv1d(128, 256, kernel_size=15, stride=1, padding=7),  # Slightly narrower kernel
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2)
        )
        ...
        self.fc1 = nn.Linear(512 * (input_dim // 16), 512)
...
model_mitbih = WideKernelCNN(input_dim, num_classes_mitbih).to(device)
model_ptbdb = WideKernelCNN(input_dim, num_classes_ptbdb).to(device)
